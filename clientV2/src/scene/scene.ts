...
import { Knight, Builder } from "../guys/guys"
...
export type Command = MoveUnitCommand | PlaceBuildingCommand
...
export class Scene {
	...
	currentBuildingType: BuildingType | false = false
	mouseX: number = 0
	mouseY: number = 0
	modelsDict: any
	zoom: number = 1
	...
	cubes: THREE.Mesh[] = []
	constructor(containerId: string, models: any, commandBuffer: Command[]) {
		this.keysPressed = {}
		this.buildings = []
		this.playerId = -1
		this.moveType = 0
		...
		if (this.isBuilding && this.currentBuildingType) {
			if (this.canBuild) {
				if (mouseButton == 0 && clickLocation) {
					...
					console.log("commandBuffer", this.commandBuffer)
					this.isBuilding = false
					return
				} else {
					// Other click
